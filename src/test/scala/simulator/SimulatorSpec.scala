...
class SimulatorSpec extends BaseSpec {
  ...
  class Spec(val numInputs: Int, val spec: BuilderEnv ?=> Instance)
  ...
  def genCompositeSpec(maxDepth: Int): Gen[Spec] = for {
    ...
    Spec(
      childSpecsFuncs.map(_.numInputs).sum, {
        val Instance(comb0, f0) = spec.spec
        val children = childSpecsFuncs.map { spec => (spec.numInputs, spec.spec) }
